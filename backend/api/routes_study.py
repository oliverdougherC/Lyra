...
def _study_sources(
    ...
    """The ready document ids to generate from: the named ones, or the whole class.

    Every named document must belong to the class and be ready. A document the class
    does not contain is a 404, and one still processing is named in the 409, because
    both are things the student can act on.
    """
    rows = conn.execute(
    ...
    if document_ids is not None:
        ...
        chosen = [by_id[document_id] for document_id in document_ids]
        ready = [int(row["id"]) for row in chosen if row["state"] == "ready"]
        if not ready:
            raise ConflictError(NAMED_NOT_READY_MESSAGE)
        return ready
...
def create_deck(class_id: int, payload: DeckCreate, conn: DbConn) -> dict[str, object]:
    created = _create_study_artifact(
        conn, class_id, artifacts.KIND_FLASHCARD_DECK, payload.title, payload.document_ids
    )
    study.enqueue(study._Job(int(created["id"]), cards_per_topic=payload.cards_per_topic))
    return created
...
def review_card(part_id: int, payload: CardReview, conn: DbConn) -> dict[str, object]:
    """Apply one rating through the scheduler and log it."""
    part, artifact = _require_card(conn, part_id)
    if artifact["state"] != artifacts.READY:
        raise ConflictError(DECK_NOT_READY_MESSAGE)
    ...
    conn.execute(
        "insert into card_review_log (part_id, rating) values (?, ?)",
        (part_id, payload.rating),
    )
    conn.commit()
    refreshed = conn.execute("select * from card_states where part_id = ?", (part_id,)).fetchone()
    return _state_json(refreshed)
...
def start_attempt(artifact_id: int, conn: DbConn) -> dict[str, object]:
    ...
    attempt_id = int(
        conn.execute("insert into quiz_attempts (artifact_id) values (?)", (artifact_id,)).lastrowid
        or 0
    )
    conn.commit()
    return {"attempt_id": attempt_id, "question_part_ids": part_ids}


def _require_attempt(conn: sqlite3.Connection, attempt_id: int) -> sqlite3.Row:
...
def answer_question(attempt_id: int, payload: AnswerCreate, conn: DbConn) -> dict[str, object]:
    """Grade one answer against the stored payload and record it."""
...
def finish_attempt(attempt_id: int, conn: DbConn) -> dict[str, object]:
    """Close an attempt and score it, per topic: the weakness surface."""
    attempt = _require_attempt(conn, attempt_id)
    if attempt["finished_at"] is not None:
        raise ConflictError(ATTEMPT_FINISHED_MESSAGE)
    conn.execute(
    ...
    score = sum(int(row["correct"]) for row in rows)
