...
export function DesktopBackupSection({ unsavedSettings }: { unsavedSettings: boolean }) {
  ...
  return (
    <section className="space-y-3" aria-labelledby="desktop-backup-title">
      <h2 id="desktop-backup-title" className="font-display text-xl">
        Backup and restore
      </h2>
      ...
      <div className="flex flex-wrap gap-2">
        <Button variant="outline" disabled={busy} onClick={() => void run('desktop_backup_create')}>
          Save backup
        </Button>
        <Button
          variant="outline"
