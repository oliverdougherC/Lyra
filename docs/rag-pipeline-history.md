# Historical RAG research and phase specification

> Archived September 6, 2026. This preserves prior experiments, measurements, and planned
> runtimes. Commands, provider choices, and future-phase statements below are not current
> setup guidance. Use [the maintained pipeline](rag-pipeline.md) and
> [architecture](architecture.md) for present behavior.
...
- The current question is appended verbatim and never trimmed, so it is charged against the budget
  ...
  context rather than pushing the prompt past the window.
- History keeps its own share, and retrieved context receives whatever the window still holds once the
